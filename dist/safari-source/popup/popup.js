(function initPopup(root) {
  ...
  function render() {
    ...
    elements.enabledToggle.checked = currentSettings.enabled && enabled;
    ...
    elements.statusBadge.classList.toggle("is-on", enabled && !excluded);
    ...
    elements.captureTabButton.disabled = status.canCaptureTab === false;
    elements.captureTabButton.textContent = status.sourceType === "tab-capture"
      ? i18n("popupStopCapture", "Stop capture")
      : i18n("popupCaptureTab", "Capture tab");
  ...
  async function setEnabled(enabled) {
    ...
    const messageType = enabled ? "WLG_ACTIVATE_CURRENT_TAB" : "WLG_DEACTIVATE_CURRENT_TAB";
  ...
  async function requestTabCapture() {
    const type = currentStatus && currentStatus.sourceType === "tab-capture"
      ? "WLG_STOP_TAB_CAPTURE"
      : "WLG_START_TAB_CAPTURE";
  ...
  elements.captureTabButton.addEventListener("click", () => {
    requestTabCapture();
  });
